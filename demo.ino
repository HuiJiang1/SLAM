...
const int LEFTMOST_PHOTO = A12;
const int MIDLEFT_PHOTO = A13;
const int MIDRIGHT_PHOTO = A14;
const int RIGHTMOST_PHOTO = A15;

const int FAN_PIN = 8;
const int NINEGEAR_PIN = 9;

Phototransistor first('L', LEFTMOST_PHOTO);
Phototransistor second('B', MIDLEFT_PHOTO);
Phototransistor third('B', MIDRIGHT_PHOTO);
Phototransistor fourth('L', RIGHTMOST_PHOTO);

Fan fan(FAN_PIN);
NineGearServo nineGear(NINEGEAR_PIN);

void setup(void)
{
  Serial.begin(115200);
  pinMode(FAN_PIN, OUTPUT);
  NineGear.attach(NINEGEAR_PIN);
}
...
void running() 
{
  static STATE state = NO_FIRE;
  static unsigned long previous_millis;
  static unsigned long tprevious_millis;
  static unsigned long fprevious_millis;

  if (millis() - tprevious_millis > 10) {
    tprevious_millis = millis();

    first.tenMillisecFunc();
    second.tenMillisecFunc();
    third.tenMillisecFunc();
    fourth.tenMillisecFunc();

    if (state = FIRE) {
        if ((first.getAvg() > second.getAvg()) || (second.getAvg() > third.getAvg()))
          nineGear.direct(false);    
        if ((fourth.getAvg() > third.getAvg()) || (third.getAvg() > second.getAvg()))
          nineGear.direct(true);
    }
  }

  if (millis() - fprevious_millis > 500) {
    fprevious_millis = millis();
    
    first.fiveHundredMillisecFunc();
    second.fiveHundredMillisecFunc();
    third.fiveHundredMillisecFunc();
    fourth.fiveHundredMillisecFunc();

    switch (state) {
      case NO_FIRE:
        if ((first.getAvg() || second.getAvg() || third.getAvg() || fourth.getAvg()) > 1.3) {
          fan.needsToBeOn(true);
          state = FIRE;
        }
        else
          state = NO_FIRE;
    ...
    Serial.println("----------R U N N I N G----------");
    Serial.print("Photo Trans One Reading: ");
    Serial.println(first.getAvg());
    Serial.print("Photo Trans Two Reading: ");
    Serial.println(second.getAvg());
    Serial.print("Photo Trans Three Reading: ");
    Serial.println(third.getAvg());
    Serial.print("Photo Trans Four Reading: ");
    Serial.println(fourth.getAvg());
  }
}
